...
- 已实现「专注查看模式」：
  ...
  - 进入后自动适配视口，`Esc` 可退出
- 已完成「Mermaid 大文本稳定性与换行」：
  - 提升 Mermaid `maxTextSize` 上限，降低长文本渲染失败概率
  - 节点/标签支持自动换行，且节点容器高度与文本行数同步
  - 渲染失败时，源码回退块也支持换行
...
1. **Mermaid 大文本稳定性与换行（已完成）**
   - ✅ 提升 Mermaid `maxTextSize` 上限，避免长文本直接失败
   - ✅ 保证节点/标签长文本自动换行（含中英文长词场景）
   - ✅ 渲染失败时，源码块也可换行，避免阅读困难

2. **缩放交互策略固定化（不做配置）**
   - 保持「仅 `Ctrl/Cmd + 滚轮` 才触发缩放」
   - 普通滚轮始终用于页面/容器滚动，避免误触
   - 缩放步长与最小/最大倍率维持固定值
...
推荐先做：**3 + 4**  
理由：缩放交互规则已明确，下一步优先补键盘可操作性与大文档性能。
